...
let direction = {
    ...
    draw: function () {
        ...
        ctx.fillText("Y", this.projectionPoints[3][0], this.projectionPoints[3][1]);
        ctx.fillText("Z", this.projectionPoints[5][0], this.projectionPoints[5][1]);
    }
};
...
function intersectionView(p1, p2) {
    ...
    let directions = [p2.x + p1.x < 0, p2.y + p1.y < 0, p2.z + p1.z < 0];
    ...
    while (intersectionIndex < 6) {
        ...
        let pointOfIntersection = linePlaneIntersection(p1, p2, cubePlanes[intersectionIndex + planeDirection]);
